...
async function getOverride(rel){let b;if(OVERRIDE_DIR){b=fs.readFileSync(path.join(OVERRIDE_DIR,...rel.split('/')))}else b=await download(RAW+rel.split('/').map(encodeURIComponent).join('/'));const got=sha(b),want=MANIFEST[rel];if(got!==want)die(`OVERRIDE_HASH_FAIL ${rel} got=${got} want=${want}`);return b}
